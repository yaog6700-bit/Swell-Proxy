...
namespace AnywhereWinUI.ViewModels
{
    ...
    public partial class ServersViewModel : ObservableObject
    {
        ...
        public void ApplyFilters()
        {
            ...
            {
                query = query.Where(s => string.IsNullOrEmpty(s.SubscriptionId));
            }
        ...
        private void RebuildGroupFilters()
        {
            ...
            if (AllServers.Any(s => string.IsNullOrEmpty(s.SubscriptionId)))
            {
                GroupFilters.Add(new ServerGroupFilterItem { Id = ManualFilterId, Name = "自建服务器" });
            }
            ...
            foreach (var orphanId in AllServers
                         .Select(s => s.SubscriptionId)
                         .Where(id => !string.IsNullOrEmpty(id))
                         .Distinct(StringComparer.Ordinal)
                         .Where(id => !Subscriptions.Any(sub => sub.Id == id)))
            {
                GroupFilters.Add(new ServerGroupFilterItem { Id = orphanId, Name = "已删除订阅" });
            }
